...
export const updateUserData = async (req, res) => {
  try {
    ...
    if (tempUser.username !== username) {
      ...
      if (userExists) {
        username = tempUser.username; // Don't change if taken
      }
    }
...
// Get User Connections, Followers, and Following (New Combined Route)
export const getFollowersFollowing = async (req, res) => {
  try {
    const { userId } = req.auth();

    const user = await User.findById(userId)
      .populate("followers", "_id full_name username profile_picture")
      .populate("following", "_id full_name username profile_picture");
